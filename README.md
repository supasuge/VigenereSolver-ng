...
- [Part I — Mathematical model](#part-i--mathematical-model)
	...
	- [11. Hyperparameter tuning (softmax MLE on the posterior)](#11-hyperparameter-tuning-softmax-mle-on-the-posterior)
...
- [Part II — Code map (function-by-function)](#part-ii--code-map-function-by-function)
	- [`vigenere.alphabet` — cipher primitives](#vigenerealphabet--cipher-primitives)
	- [`vigenere.stats` — distributional statistics](#vigenerestats--distributional-statistics)
	- [`vigenere.language` — n-gram tables](#vigenerelanguage--n-gram-tables)
	- [`vigenere.keylength` — key-length estimation (§3, §4, §5, §6)](#vigenerekeylength--key-length-estimation-%C2%A73-%C2%A74-%C2%A75-%C2%A76)
	- [`vigenere.scoring` — pluggable scorers (§8)](#vigenerescoring--pluggable-scorers-%C2%A78)
	- [`vigenere.search` — beam search (§7)](#vigeneresearch--beam-search-%C2%A77)
	- [`vigenere.refine` — wordlist refinement (§10)](#vigenererefine--wordlist-refinement-%C2%A710)
	- [`vigenere.match` — key-match classification (§13)](#vigenerematch--key-match-classification-%C2%A713)
	- [`vigenere.tune` — posterior weight learner (§11)](#vigeneretune--posterior-weight-learner-%C2%A711)
	- [`vigenere.optimize` — Pareto parameter search (§12)](#vigenereoptimize--pareto-parameter-search-%C2%A712)
	- [`vigenere.progress` — live progress UI](#vigenereprogress--live-progress-ui)
	- [`vigenere.solver` — orchestration](#vigeneresolver--orchestration)
	- [`vigenere.bench` — corpus generation + harness](#vigenerebench--corpus-generation--harness)
	- [`vigenere.cli` — `argparse` subcommands](#vigenerecli--argparse-subcommands)
...
- [Adaptive `solve_auto`](#adaptive-solve_auto)
- [Optimizations & ML strategy](#optimizations--ml-strategy)
	- [Learned key-length posterior (§11)](#learned-key-length-posterior-%C2%A711)
	- [Pareto parameter search (§12)](#pareto-parameter-search-%C2%A712)
	- [Ensemble decoder (§8)](#ensemble-decoder-%C2%A78)
	- [Confidence + match heuristics (§9, §13)](#confidence--match-heuristics-%C2%A79-%C2%A713)
